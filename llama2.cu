...
int load_weights(void *w, int elements, FILE* f, void *scratchCPU, void *scratchGPU) {
    int count;
    int float_size = (scratchGPU != nullptr) ? sizeof(float) : sizeof(half);
    count = fread(scratchCPU, float_size, elements, f);
    if (count != elements) return 1;
    // copy and convert fp32->fp16
    if (scratchGPU != nullptr) {
        // copy and convert fp32->fp16
...
int checkpoint_init_weights(TransformerWeights* w, Config* p, FILE* f, int shared_weights, char *checkpoint) {
    int head_size = p->dim / p->n_heads;
    ...
    // Check if it's a .f16 model file
    int is_fp16=0;
    {
        char *c = checkpoint;
        while (*c) c++;
        is_fp16 = (c >= checkpoint + 4) && (c[-1] == '6' && c[-2] == '1' && c[-3] == 'f' && c[-4] == '.');
    }
    if (!is_fp16) cudaMalloc(&scratchGPU, scratch_size);
    ...
    if (scratchGPU != nullptr) cudaFree(scratchGPU);
    free(scratchCPU);
    return 0;
}
...
int main(int argc, char *argv[]) {
    ...
    int shared_weights;
    {
        ...
        if (checkpoint_init_weights(&weights, &config, file, shared_weights, checkpoint)) { return 1; }
    }
